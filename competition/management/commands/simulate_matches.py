...
def calculate_elo_rating(rating_a, rating_b, score_a, score_b, k_factor=32):
    ...
    new_rating_a = max(rating_a + k_factor * (score_a - expected_a), BASE_RATING)
    new_rating_b = max(rating_b + k_factor * (score_b - expected_b), BASE_RATING)
...
def simulate_matches(competitions=None):
    ...
    categories = CompetitionCategory.objects.filter(competition__in=competitions)
    ...
    for competition in competitions:
        for category in categories:
            print(f"Simulating matches for {category}")
            all_members = list(
                Member.objects.filter(weight_class=category.weight_class)
            )
            all_matches = []
            ...
            if len(all_members) % 2 != 0:
                ...
                all_members.append(extra_match.matchmember_set.get(status=1).member)
            ...
            if len(all_members) > 2:
                ...
                for match in matches:
                    semifinal_losers.append(match.matchmember_set.get(status=2).member)
            ...
            if len(semifinal_losers) == 2:
                third_place_match = create_match(
                    ...
                    semifinal_losers[1],
                    4,
                )  # Assume round 6 is for third-place
...
def create_round(category, judges, members, round_number, count_after):
    ...
    while len(members) > count_after:
        for member1, member2 in combinations(members, 2):
            match = create_match(
                category, judges, member1, member2, round_number, match_time
            )
            matches.append(match)
            members.remove(member1)
            members.remove(member2)
            members.append(match.matchmember_set.get(status=1).member)
            match_time += time_difference
            break
    return matches, members


def create_match(category, judges, member1, member2, round_number, match_time=None):
    ...
    match = Match.objects.create(
        match_time=match_time or timezone.now(),
    ...
    print(
        f"Match created in round {round_number}: {member1.full_name} vs {member2.full_name} ({score_a}:{score_b}), Winner: {member1.full_name if score_a > score_b else member2.full_name}"
    )
